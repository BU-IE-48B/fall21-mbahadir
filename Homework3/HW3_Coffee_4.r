...
dist_dtw_test=as.matrix(dtwDist(all_dt))
diag(dist_dtw_test)=large_number

last_result=nn_classify_cv(dist_dtw_test,allclass,test_indices_last,k=last_k)
accuracy=sum(allclass[test_indices_last]==last_result$prediction$predicted)/length(test_indices_last)
final_res=data.table(approach="Coffee_dtw_raw_Test.csv", k=last_k, acc=accuracy)
